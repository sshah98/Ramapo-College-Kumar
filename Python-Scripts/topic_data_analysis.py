...
fileName = "ArithmeticVizExpln" #copy/paste filename without file extension
file = "%s.xlsx" % (fileName) #creates file variable for rest of program

path = r'/home/suraj/Documents/Ramapo-Project/ArithmeticVizExpln.xlsx' #path to copy loaded workbook to
...
def allgrades():
    ...
    print("Saving data to allgrades sheet")
    writer = pd.ExcelWriter(path, engine='openpyxl')
    book = load_workbook(path)
    writer.book = book
    writer.sheets = dict((ws.title, ws) for ws in book.worksheets)
...
def correct():
    ...
    print("Opening allgrades dataframe sheet")
...
def conditional():
    ...
    print("Saving data to Conditional Sheet")
    writer = pd.ExcelWriter(path, engine='openpyxl')
    book = load_workbook(path)
    writer.book = book
    writer.sheets = dict((ws.title, ws) for ws in book.worksheets)
...
def cumulative():
    ...
    print("Saving data to Cumulative Sheet")
    writer = pd.ExcelWriter(path, engine='openpyxl')
    book = load_workbook(path)
    writer.book = book
    writer.sheets = dict((ws.title, ws) for ws in book.worksheets)
