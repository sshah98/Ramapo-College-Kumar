...
def readProbletResultData(filename, byTemplate=False):
  ...
  False (the default) then the problems are stored by problet number.
  When 'byTemplate' is True then the problems are stored by template
  number.  There is no filtering based on problet type or whether or
  ...
  for line in probletResultLines:
    ...
    if not studentID in studentResultDict:
      studentResultDict[studentID] = probletResults
    else:
      probletResults = studentResultDict[studentID]

    # Add all problet results you find to the current
    # dictionary.  Results come in groups of six fields
    for idx in xrange(2,len(fields),6):
      if (idx+5) < len(fields):
        rec = {}
        ...
        rec['problet'] = int(fields[idx])
        rec['template'] =  int(fields[idx+1])
        rec['attempted'] = int(fields[idx+2])
        rec['correct'] = int(fields[idx+3])
        rec['total'] = int(fields[idx+4])
        rec['time'] = int(fields[idx+5])
        
        probletResults[problemIndex] = rec
...
def printProbletDataSet(dataset):
  """
  A convenience function to print a student/problet dataset in a
  'pretty' way.
  """
  print "Student ID".ljust(10), "Problet/Template ID"
  print "----------",           "-------------------"
...
def cleanupForStudentAnalysis(results, probletSubset):
  ...
  Filter the data for doing student-based analysis:
    * Strip out students for whom there's no response on
      the specified problets;
    * Set the 'testResult' to True for each problet that
  ...
  students.sort()
  problets = list(probletSubset)
  problets.sort()

  # For each problet and student, give a 'True' if the student
  # got *all* instances of that problet correct and 'False' otherwise.
  testDict = {}
  for student in students:
    resultList = []
    for problet in problets:
      numCorrect = results[student][problet]['correct']
      numTotal   = results[student][problet]['total']
      resultList.append(numCorrect >= numTotal)
...
def cleanupForProbletAnalysis(results, probletSubset):
  """
  Filter the data for doing problet-based analysis:
    * Strip out students for whom there's no response on
      the specified problets;
    * Set the 'testResult' to True for each problet that
      the student got entirely correct;
    * Organize as a list of tuples, where each tuple is a problet and
      the tuple is the vector of results of what students solved
      that problet correctly;
    * Throw out duplicates of specific result configurations.
  """
  # Use only students that have taken the specified problets
  students = []
  ...
  for problet in problets:
    ...
    # Ensures only one copy of tests with a specific result configuration
    # is retained (i.e., reserves uniqueness of result)
    testDict[testResult] =  (testResult, sum(testResult), problet) 
...
def generateFakeData(n=5, d=2):
  ...
  playing the the compare-on-one game betwee many candidate
...
def filterTests(tests):
  ...
  in configuration of results and also sorts them in order ot the number of
  successful tests.
  """
...
def summarizeProbletAnalysis(dims, byTemplate=False):
  """
  The result will be a print out of each separate problet dimension, with each
  problet in that dimension listed in order of Pareto dominance (the top one
...
def summarizeStudentAnalysis(dims):
  ...
  dominating the others, etc).  Also listed are the number of problets passed
...
def summarizeMaxMinDimensions(dims, testSubset):
  ...
  This function attempts to provide some feedback about difficulty.  It optimizes two things.
    1. It looks across all dimensions of *learners*, finds the best learner in that dimension,
       the keeps track of which problets were *missed* by these learner.  It reports how many
    ...
    2. It looks across all dimensions of *learners*, finds the worst learner in that dimension,
       the keeps track of which problets were *passed* by these learner.  It reports how many
...
if __name__ == '__main__':
  ...
  print "  The problets in that dimension are listed in Pareto dominance order, such that the"
  print "  top problet strictly dominates the ones under it.  Only the top-most problet in"
  ...
  print "Problet Difficulty Analysis (*):".upper()
  print ''.ljust(70,'-')
  print "  For the 'Hard' row, we count the number of times the *best* student"
  print "  in each dimension misses that Problet.  The bigger the number, the harder."
  print
  print "  For the 'easy' row, we count the number of times the *worst* student"
  print "  in each dimension passes that Problet.  The bigger the number, the easier."
  print
